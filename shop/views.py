...
#TODO:DjangoRESTFrameworkを使うため、django.viewsではなくrest_frameworkのviews.APIViewを継承してクラスを作る。これで、PUT,DELETE,PATCHメソッドを受け付ける。
...
class PatternView(views.APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        #TODO:ここはOR検索をするためクエリビルダを使う、|でOR検索、&でAND検索
...
class PatternModView(LoginRequiredMixin,views.APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if "pk" in kwargs:
            #TODO:ここは自分が投稿したデータだけ、存在しない場合はリダイレクト
    ...
    def put(self, request, *args, **kwargs):
        ...
        #TODO:レシピ登録で違うのはここだけ。ここでレシピの編集を行う直前、編集前のレシピは全て削除する。
